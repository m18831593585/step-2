...
export default class Form extends Component {
    ...
    pushPersonInfo(person) {
        ...
        _person.hobby = _person.hobby.join(';')
        for (let item in _person) {
            _arr.push(_person[item])
        }
        let _ajax = new XMLHttpRequest()
        _ajax.open('POST', `http://localhost:8080/insert`)
        _ajax.send(_arr)
    }

    ajaxLoadHandler() {
        let _list = JSON.parse(this.ajax.response).data
        this.showTable.innerHTML = _list.reduce((value, item) => {
            return value + `
                    <tr>
                        ...
                        <td>${item.hobby}</td>
                        <td>${item.tel}</td>
                        <td><button class="delete-btn" data="${item.id}">删除</button></td>
                    </tr>
                `
        }, '')
        if (!this.showTable.querySelector('.delete-btn')) return
        this.showTable.querySelectorAll('.delete-btn').forEach(item => {
            item.addEventListener('click', e => this.deleteHandler(e))
        })
    }
    ...
    deleteHandler(e) {
        ...
        let _ajax = new XMLHttpRequest()
        _ajax.open('POST', `http://localhost:8080/delete`)
        _ajax.send(id)
        this.render()
    }
    ...
    render() {
        ...
        this.ajax.open('GET', 'http://localhost:8080/select?sex=' + this.currentSex)
        this.ajax.addEventListener("load", () => this.ajaxLoadHandler())
        this.ajax.send()
    }
